...
(
  ...
  SELECT
    ...
    WHERE I.ITEM_ID = P.ITEM_ID AND P.CUSTOMER_ID = IN_CUSTOMER_ID;
  END PROC_PURCHASE_LIST;
  
--  PROCEDURE PROC_PURCHASE_ADD(IN_CUSTOMER_ID IN VARCHAR2) AS
--       V_CNT NUMBER(5);   
--  BEGIN
--SELECT COUNT(BASKET_ID) INTO V_CNT FROM BASKET;  
--LOOP
-- INSERT INTO PURCHASE(PURCHASE_ID,CNT) VALUES( seq.nextval,1) ;
-- V_CNT := V_CNT -1;
-- EXIT WHEN V_CNT<0;
-- END LOOP; 
--    INSERT INTO PURCHASE(ITEM_ID, CUSTOMER_ID, CNT, REGDATE) 
--       SELECT ITEM_ID, CUSTOMER_ID, CNT, REGDATE FROM BASKET WHERE CUSTOMER_ID =  IN_CUSTOMER_ID;
--    COMMIT;
...
PROCEDURE PROC_MAIN_VIEW (O_CUR  OUT SYS_REFCURSOR) AS 
  ...
  BEGIN
    ...
    SELECT ITEM_ID, ITEM_NAME, PRICE, BUYCOUNT
    FROM 
    (SELECT  ITEM_ID, ITEM_NAME, PRICE, BUYCOUNT 
    FROM  ITEM
